...
gcai() {
    ...
    commit_msg=$(gum spin --spinner dot --title "Generating commit message..." -- zsh -c 'git --no-pager diff --cached | agy --prompt "Summarize this change in a concise git commit message. Follow Git semantic commit conventions and standards. Raw text no markdowns" 2>/dev/null')
    if [ -z "$commit_msg" ]; then
