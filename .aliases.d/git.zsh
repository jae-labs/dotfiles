...
# Clean up the repository by switching to the main branch, deleting all other branches, expiring reflog entries, performing garbage collection, and removing untracked files.
alias gcleanff="(git checkout main 2>/dev/null || git checkout master) && git fetch -p && git pull && git branch --format '%(refname:short)' | grep -vE '^(main|master)$' | xargs git branch -D 2>/dev/null; git reflog expire --expire=now --all; git gc --prune=now; git clean -xdf"
...
gnukehistory() {
  ...
  git add -A && \
  git commit -am "feat: implement dreams and technical debt" && \
...
gcai() {
    ...
    commit_msg=$(gum spin --spinner dot --title "Generating commit message..." -- zsh -c '
  git --no-pager diff --cached | claude -p --model claude-haiku-4-5 \
  "Generate a multiline git commit message based on this diff:
  1. First line: <type>(<optional-scope>): <summary> (under 50 chars).
  2. Allowed types: maint, fix, feat, build, chore, ci, docs, style, refactor, revert, perf, plan, test.
  3. Leave line 2 blank.
  4. Remaining lines: 1-3 bullet points listing essential body details.
  Raw text only: no markdown, code blocks, backticks, quotes, or preambles." 2>/dev/null
')
    if [ -z "$commit_msg" ]; then
